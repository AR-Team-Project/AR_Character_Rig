...
function onResults2(results) {
  ...
  let poseRig = Kalidokit.Pose.solve(poselm3d, results.poseLandmarks, {
    runtime: "mediapipe",
    imageSize: { width: 820, height: 462 },
  });
  ...
  let matT_leftUpLeg = new THREE.Matrix4().makeTranslation(
    mixamor_leftUpLeg.position.x,
    mixamor_leftUpLeg.position.y,
    mixamor_leftUpLeg.position.z
  );
  let mixamor_leftLeg = skeleton.getBoneByName("mixamorigLeftLeg");
  let matT_leftLeg = new THREE.Matrix4().makeTranslation(
    mixamor_leftLeg.position.x,
    mixamor_leftLeg.position.y,
    mixamor_leftLeg.position.z
  );
  let mixamor_rightUpLeg = skeleton.getBoneByName("mixamorigRightUpLeg");
  let matT_rightUpLeg = new THREE.Matrix4().makeTranslation(
    mixamor_rightUpLeg.position.x,
    mixamor_rightUpLeg.position.y,
    mixamor_rightUpLeg.position.z
  );
  let mixamor_rightLeg = skeleton.getBoneByName("mixamorigRightLeg");
  let matT_rightLeg = new THREE.Matrix4().makeTranslation(
    mixamor_rightLeg.position.x,
    mixamor_rightLeg.position.y,
    mixamor_rightLeg.position.z
  );
  ...
  let sv_matR_leftupleg = new THREE.Matrix4().makeRotationFromEuler(
    sv_euler_leftupleg
  );
  let sv_matR_leftleg = new THREE.Matrix4().makeRotationFromEuler(
    sv_euler_leftleg
  );
  let sv_matR_rightupleg = new THREE.Matrix4().makeRotationFromEuler(
    sv_euler_rightupleg
  );
  let sv_matR_rightleg = new THREE.Matrix4().makeRotationFromEuler(
    sv_euler_rightleg
  );
  ...
  mixamor_rightUpLeg.matrix.multiplyMatrices(
    matT_rightUpLeg,
    sv_matR_rightupleg
  );
